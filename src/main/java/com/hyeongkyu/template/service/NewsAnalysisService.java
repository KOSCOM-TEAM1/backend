...
import com.hyeongkyu.template.domain.dto.request.AnalyzeNewsRequest;
...
public class NewsAnalysisService {
    ...
    private NewsAnalysisResponse analyzeNewsWithAI(News news) {
        ...
        ChatCompletionRequest chatRequest = ChatCompletionRequest.builder()
                .model("gpt-4")  // gpt-4 또는 gpt-3.5-turbo 사용 가능
                .messages(messages)
                .temperature(0.7)
                .maxTokens(2000)
                .build();
    ...
    private String buildAnalysisPrompt(News news) {
        ...
        prompt.append("내용: ").append(news.getContent()).append("\n");
        prompt.append("출처: ").append(news.getSource()).append("\n");
        prompt.append("발행 시간: ").append(news.getPublishedAt()).append("\n\n");
    ...
    private List<News> findSimilarNews(News currentNews) {
        ...
        return recentNews.stream()
                .filter(news -> !news.getId().equals(currentNews.getId()))
                .limit(3)
                .collect(Collectors.toList());
    }
